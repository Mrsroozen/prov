...
async function getInfo() {
    ...
    data.forEach((posts) => {
        ...
        postsEl.style.fontWeight = "bold";
        postsEl.style.color = "gray";
        postsDiv.appendChild(postsEl);
       
        
        const bodyEl = document.createElement("div");
        bodyEl.innerHTML = `<p>${posts.body}</p>`;
        bodyEl.style.color = "gray";
        bodyEl.style.backgroundColor = "lightgray";
        bodyEl.style.padding = "20";
        bodyEl.style.margin = "10";

        postsDiv.appendChild(bodyEl);
    }) 
...
const postsDivWithComments = document.querySelector("#postsWithComments");

async function getInfoWithComments() {
    const response = await fetch("https://jsonplaceholder.typicode.com/comments");
    const data = await response.json();

    data.forEach((comments) => {
        const commentsEl = document.createElement("div");
        commentsEl.innerHTML = `<p>${comments.body}</p>`;
        commentsEl.style.color = "gray";
        commentsEl.style.backgroundColor = "lightgray";
        commentsEl.style.padding = "20";
        commentsEl.style.margin = "10";
       
        postsDivWithComments.appendChild(commentsEl);
})
}
